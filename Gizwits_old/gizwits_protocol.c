...
static int8_t ICACHE_FLASH_ATTR gizDataPoint2Event(gizwitsIssued_t *issuedData, eventInfo_t *info, dataPoint_t *dataPoints)
{
    ...
    if(0x01 == issuedData->attrFlags.flagbeep_flag)
    {
        info->event[info->num] = EVENT_beep_flag;
        info->num++;
        dataPoints->valuebeep_flag = gizStandardDecompressionValue(beep_flag_BYTEOFFSET,beep_flag_BITOFFSET,beep_flag_LEN,(uint8_t *)&issuedData->attrVals.wBitBuf,sizeof(issuedData->attrVals.wBitBuf));
    }
        
        
    if(0x01 == issuedData->attrFlags.flagtmp_threshold)
    {
        info->event[info->num] = EVENT_tmp_threshold;
        info->num++;
        dataPoints->valuetmp_threshold = gizX2Y(tmp_threshold_RATIO,  tmp_threshold_ADDITION, issuedData->attrVals.valuetmp_threshold); 
    }
        
    if(0x01 == issuedData->attrFlags.flaghum_threshold)
    {
        info->event[info->num] = EVENT_hum_threshold;
        info->num++;
        dataPoints->valuehum_threshold = gizX2Y(hum_threshold_RATIO,  hum_threshold_ADDITION, issuedData->attrVals.valuehum_threshold); 
    }
        
    if(0x01 == issuedData->attrFlags.flagfire_threshold)
    {
        info->event[info->num] = EVENT_fire_threshold;
        info->num++;
        dataPoints->valuefire_threshold = gizX2Y(fire_threshold_RATIO,  fire_threshold_ADDITION, exchangeBytes(issuedData->attrVals.valuefire_threshold));
    }
        
    if(0x01 == issuedData->attrFlags.flagmq2_threshold)
    {
        info->event[info->num] = EVENT_mq2_threshold;
        info->num++;
        dataPoints->valuemq2_threshold = gizX2Y(mq2_threshold_RATIO,  mq2_threshold_ADDITION, exchangeBytes(issuedData->attrVals.valuemq2_threshold));
    }
...
static int8_t ICACHE_FLASH_ATTR gizCheckReport(dataPoint_t *cur, dataPoint_t *last)
{
    ...
    if(last->valuebeep_flag != cur->valuebeep_flag)
    {
        GIZWITS_LOG("valuebeep_flag Changed\n");
        ret = 1;
    }
    if(last->valuetmp_threshold != cur->valuetmp_threshold)
    {
        GIZWITS_LOG("valuetmp_threshold Changed\n");
        ret = 1;
    }
    if(last->valuehum_threshold != cur->valuehum_threshold)
    {
        GIZWITS_LOG("valuehum_threshold Changed\n");
        ret = 1;
    }
    if(last->valuefire_threshold != cur->valuefire_threshold)
    {
        GIZWITS_LOG("valuefire_threshold Changed\n");
        ret = 1;
    }
    if(last->valuemq2_threshold != cur->valuemq2_threshold)
    {
        GIZWITS_LOG("valuemq2_threshold Changed\n");
        ret = 1;
    }
    if(last->valuetmp_flag != cur->valuetmp_flag)
    {
        GIZWITS_LOG("valuetmp_flag Changed\n");
        ret = 1;
    }
    if(last->valuehum_flag != cur->valuehum_flag)
    {
        GIZWITS_LOG("valuehum_flag Changed\n");
        ret = 1;
    }
    if(last->valuefire_flag != cur->valuefire_flag)
    {
        GIZWITS_LOG("valuefire_flag Changed\n");
        ret = 1;
    }
    if(last->valuesmoke_flag != cur->valuesmoke_flag)
    {
        GIZWITS_LOG("valuesmoke_flag Changed\n");
        ret = 1;
    }

    if(last->valuehum_data != cur->valuehum_data)
    {
        if(currentTime - lastReportTime >= REPORT_TIME_MAX)
        {
            GIZWITS_LOG("valuehum_data Changed\n");
            ret = 1;
        }
    }
    if(last->valuetmp_data != cur->valuetmp_data)
    {
        if(currentTime - lastReportTime >= REPORT_TIME_MAX)
        {
            GIZWITS_LOG("valuetmp_data Changed\n");
            ret = 1;
        }
    }
    if(last->valuefire_data != cur->valuefire_data)
    {
        if(currentTime - lastReportTime >= REPORT_TIME_MAX)
        {
            GIZWITS_LOG("valuefire_data Changed\n");
            ret = 1;
        }
    }
    if(last->valuemq2_data != cur->valuemq2_data)
    {
        if(currentTime - lastReportTime >= REPORT_TIME_MAX)
        {
            GIZWITS_LOG("valuemq2_data Changed\n");
            ret = 1;
        }
    }
...
static int8_t ICACHE_FLASH_ATTR gizDataPoints2ReportData(dataPoint_t *dataPoints , devStatus_t *devStatusPtr)
{
    ...
    gizStandardCompressValue(beep_flag_BYTEOFFSET,beep_flag_BITOFFSET,beep_flag_LEN,(uint8_t *)devStatusPtr,dataPoints->valuebeep_flag);
    gizStandardCompressValue(tmp_flag_BYTEOFFSET,tmp_flag_BITOFFSET,tmp_flag_LEN,(uint8_t *)devStatusPtr,dataPoints->valuetmp_flag);
    gizStandardCompressValue(hum_flag_BYTEOFFSET,hum_flag_BITOFFSET,hum_flag_LEN,(uint8_t *)devStatusPtr,dataPoints->valuehum_flag);
    gizStandardCompressValue(fire_flag_BYTEOFFSET,fire_flag_BITOFFSET,fire_flag_LEN,(uint8_t *)devStatusPtr,dataPoints->valuefire_flag);
    gizStandardCompressValue(smoke_flag_BYTEOFFSET,smoke_flag_BITOFFSET,smoke_flag_LEN,(uint8_t *)devStatusPtr,dataPoints->valuesmoke_flag);
    ...
    devStatusPtr->valuetmp_threshold = gizY2X(tmp_threshold_RATIO,  tmp_threshold_ADDITION, dataPoints->valuetmp_threshold); 
    devStatusPtr->valuehum_threshold = gizY2X(hum_threshold_RATIO,  hum_threshold_ADDITION, dataPoints->valuehum_threshold); 
    devStatusPtr->valuehum_data = gizY2X(hum_data_RATIO,  hum_data_ADDITION, dataPoints->valuehum_data); 
    devStatusPtr->valuetmp_data = gizY2X(tmp_data_RATIO,  tmp_data_ADDITION, dataPoints->valuetmp_data); 

    devStatusPtr->valuefire_threshold = exchangeBytes(gizY2X(fire_threshold_RATIO,  fire_threshold_ADDITION, dataPoints->valuefire_threshold)); 
    devStatusPtr->valuemq2_threshold = exchangeBytes(gizY2X(mq2_threshold_RATIO,  mq2_threshold_ADDITION, dataPoints->valuemq2_threshold)); 
    devStatusPtr->valuefire_data = exchangeBytes(gizY2X(fire_data_RATIO,  fire_data_ADDITION, dataPoints->valuefire_data)); 
    devStatusPtr->valuemq2_data = exchangeBytes(gizY2X(mq2_data_RATIO,  mq2_data_ADDITION, dataPoints->valuemq2_data)); 
